...
class CheckUnifiDevice(Check):
    ...
    async def run(asset: Asset, local_config: dict, config: dict) -> dict:
        ...
        item = {
            ...
            'config_network_type': config_network.get('type'),  # str/opt
            'bonding_enabled': config_network.get('bonding_enabled'),  # bool/o
        ...
        if not radio_complete:
            raise IncompleteResultException(
                'At least one radio without a name', result=state)
        if not vap_complete:
            raise IncompleteResultException(
                'At least one VAP without a name', result=state)
        if not port_complete:
            raise IncompleteResultException(
                'At least one Port without a name', result=state)
        if mac_duplicate:
